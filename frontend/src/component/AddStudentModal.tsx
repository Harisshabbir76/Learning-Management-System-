...
export default function AddStudentModal({
  ...
  const handleNewStudentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const enrollRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/sections/${sectionId}/students`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({ studentIds: [studentUserId] }),
        }
      );
  ...
  const handleExistingStudentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Enroll student using the numeric userId
      const enrollRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/sections/${sectionId}/students`,
        {
          method: 'POST',
          ...
          body: JSON.stringify({ studentIds: [numericStudentId] }),
        }
      );
